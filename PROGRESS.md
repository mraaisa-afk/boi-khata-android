...
- [x] **Part A Bengali terminology batch — IMPLEMENTED per owner rulings A1–A5 (2026-09-20 local; RE-LANDED 2026-09-24):** original Part A commits `767e190`/`743b86a` were made on PR #70's branch but **never pushed** (no token that session) — PR #70 was merged at `f1bce2d`, so main never received them, and the sandbox reset destroyed the local commits; the owner's installed build (from main `7c47ece`) therefore still showed উপমুট on the POS summary card and দেনা মুন on the khata detail button [ERR-018]. Re-land on `agent/part-a-terminology-reland` (`bba3208`): A1 উপমুট→সর্বমোট + সর্বমোট লাভ→স্থূল মুনাফা + দেনা মুন→দেনা-পাওনা (incl. confirm-copy now promising «সমন্বয়» = what KhataStatementBuilder.typeLabel:116 actually renders; stored ADJUSTMENT description renamed — no code compares it, re-verified); A2 8 rows; A3 ক্রেতা/বাকি/খরচ/খাত clusters; A4 নগদ (Nagad) at ReceiptBuilder:81 + PosScreen:550 + CashClose.kt:41; A5a ReceiptBuilder:59 dead identical-branch fix RED→GREEN (RED 36 tests/3 failed → GREEN 36/36); A5b হাটি ক্রেতা + A5c MFS-ফি untouched per ruling; adjacent disclosed typo মোত→মোট ইক্যুইটি. Composition-tolerant grep: zero replaced-term instances outside documented residuals. POS summary card (`PosScreen.kt:230 stringResource(R.string.subtotal)`) and khata button (`KhataCustomerDetailScreen.kt:272 stringResource(R.string.forgive_debt)`) render the exact keys fixed — device-visible after rebuild+reinstall
...
- [x] **P12 IMPLEMENTED (D92 build-out, 2026-09-24):** schema v6→v7 `bill_payment_lines` (ADDITIVE-ONLY — id/billId/method/provider/amount/cashbookEntryId/createdAt, indices billId+tenantId; zero data transformation; schemas exported to `core/database/schemas/` for MigrationTestHelper); D22 transaction extended to `createBillWithPaymentLines` — ONE atomic transaction writes bill + item lines + stock ledger + khata CREDIT (বাকি) + **one cashbook INCOME mirror PER PAID line** (CASH→CASH, BANK→BANK, MOBILE→**MOBILE** bucket per owner ruling — never folded into BKASH) + the payment-line rows (DUE row explicit, no cashbook); fail-fast validation (amount > 0, sum ≤ total, MOBILE requires provider, বাকি requires customer — no orphan due); legacy `createBill` delegates to the line model (NAGAD/BKASH now map to MOBILE+provider — the D34-era NAGAD→BKASH cashbook conflation is GONE); PaymentMethod gains BANK/MOBILE summary values; new PaymentLineCategory + MfsProvider (বিকাশ/নগদ (Nagad)/রকেট/উপায়/অন্যান্য — hardcoded starter set per D92 v1, upgrade path open via future D-entry); POS checkout rebuilt as a multi-line editor (per-category lines, provider chip selector per D87 chips pattern, live «বাকি থাকবে» preview, remove-line, single-blank-নগদ = full-cash legacy default); ReceiptBuilder renders one জমা row per line (2-way/3-way receipts) with legacy fallback; CashClose aggregates per line (bKash/Nagad/other-mobile/bank buckets; legacy bills fall back to bill columns; MFS fee still bKash-only — disclosed); BackupMapper embeds paymentLines INSIDE the bill document (keeps CONVENTIONS §5's 10-collection set frozen — disclosed; a future D-entry can promote it); RestoreMapper old-format tolerant; TenantRebindPlanner registry 23→24 (G16 honored); CONVENTIONS §2/§3 amended; ROOM_MIGRATION_LEDGER v7. Tests: MigrationTestHelper 6→7 under Robolectric (schema parity + legacy-data preservation + new-table insert) — RED-proven (entity indices missing → Room validation failed → fixed); SaleRepositoryImplTest +5 (2-way ৳600+৳400 zero-due, 3-way paid+paid+বাকি→khata CREDIT, 4 validation fail-fasts with zero-side-effect proof, legacy NAGAD equivalence); CashCloseCalculator +3 per-line aggregation; CashbookBalanceCalculator 4-bucket; PaymentLineBackupRoundTripTest 3 (old-format tolerant + new-format round-trip + legacy backup key-absence). Battery: 9-module lintDebug green, 14-module testDebugUnitTest green, :app:assembleDebug green [branch `agent/p12-multi-line-payments`]
- [x] **P13 DEVICE ROUND (2026-09-24, post-#73-merge): 5 owner-reported issues — all root-caused with file/line evidence before fixing (mandatory protocol).**
  **ISSUE 1 / D93 overpayment:** validation lived at `SaleRepositoryImpl.kt:207` (`require(sumPaid <= totalAmount)`, message «জমা বিলের মোটের চেয়ে বেশি হতে পারে না») + UI capped the paid preview at `SaleViewModel.recalculateTotals`. Owner ruling implemented: overpayment from a NAMED customer now posts the excess as a khata জমা (PAYMENT) entry — **inside the same D22 transaction** (block 4b), no extra cashbook row (money already mirrored via paid lines — no double-count); walk-in overpay rejected with «হাটি ক্রেতার জন্য অতিরিক্ত জমা রাখা সম্ভব নয়। অতিরিক্ত ফেরত দিন।»; bill.paidAmount = min(sum, total); bill_payment_lines keep ACTUAL received amounts; POS gets a live খাতা-জমা preview (green) or walk-in block hint (error); receipts split জমা rows + a «জমা (খাতায়)» row. RED→GREEN proven: 3 new tests fail under the old validation (10 tests/3 failed), pass after the fix. CashClose splits the excess into a new `khataAdvance` bucket so মোট বিক্রি never inflates (4 calculator tests + 13-row toLines).
  **ISSUE 2 / আজকের হিসাব labels (D94 pattern):** old «নগদ বিক্রি / বিকাশ বিক্রি / …» → new «নগদে বিক্রি / বিকাশ হতে বিক্রি / নগদ (Nagad) হতে বিক্রি / ব্যাংক হতে বিক্রি / মোবাইল ব্যাংকিং (অন্যান্য) হতে বিক্রি / বাকিতে বিক্রি» — changed in `CashClose.kt toLines()` + reports `values/`+`values-bn/`+`values-en/` strings; গোনা নগদ field placeholder → «৳ পরিমাণ লিখুন» (section label kept). grep-zero verified (residuals: test containsNoneOf guard + one test sample string only).
  **ISSUE 3 / missing বাকি posting (ইলিয়াস +৳400):** forensic result — **both** the pre-P12 and P12 write paths provably post the CREDIT (`dueAmount > 0.01 && customerId != null` → insert, same transaction; P12 test «three-way … posts the remainder to khata» GREEN at main; code cited). The device ledger state cannot be produced by any shipped code path ⇒ **data-state issue, not a live code bug**; leading hypothesis = fresh-install cloud restore of a backup whose `bills` ↔ `khata_entries` Firestore collections were out of sync (no cross-collection atomicity). Code fix shipped: **restore-time khata integrity repair** — `KhataRepairPlanner` (pure, 4 tests) + `RestoreRepositoryImpl.repairMissingKhataCredits` (idempotent, re-creates missing CREDITs for restored bills with due>0+customer+no entry). Owner remedy for existing data: record a manual বাকি +400 for ইলিয়াস, or run a fresh-device restore after this build ships (repair auto-heals IF the bills are in the cloud backup).
  **ISSUE 4 / রহিম's empty বিক্রির ইতিহাস:** `BillDao.getByCustomer` (tenantId+customerId) verified correct; every checkout path writes `customerId = selectedCustomer.id` (cited); no shared code root cause with Issue 3 (different tables), BUT both are consistent with ONE data-level event: the out-of-sync restore above (bills collection missing his docs entirely ⇒ unrepairable from code — bills absent from backup cannot be resurrected). Owner asked to confirm whether a fresh install + cloud restore happened and check his invoices in the global bill history.
  **ISSUE 5 / D94 revenue + COGS:** root causes cited: `HomeViewModel.kt:83` (revenue = Σ paidAmount — D81 cash basis; device আয় 2200 = 1200+1000 excluding বাকি 800 ✓) + `HomeScreen.kt:212` (নিট লাভ = income − expenses, no COGS) + `HomeViewModel.kt:103/122` (yesterday + month sparkline, same flaw). The monthly P&L was ALREADY compliant (`PnLCalculator.kt:57` revenue = Σ subtotal of ALL bills; COGS join at `AccountingRepositoryImpl.kt:62-77`) — proven by a new characterization test. Implemented: `BillDao.getCogsByDateRange` (SQL JOIN bill_lines×books, LEFT JOIN for deleted books → 0; real-DB Robolectric test) + `BillRepository.getCogsByDateRange`; Home revenue = Σ totalAmount; **নিট লাভ = (revenue − COGS) − expenses** (today, yesterday, month sparkline); খাতা আদায় kept as its own stat (not revenue — old dues were revenue when sold). বাকি-sale COGS explicitly counted.
  Battery: full `testDebugUnitTest` battery green; 6 touched modules lintDebug green (:core:database :core:domain :core:cloud :feature:sale :feature:home :feature:reports); :app:assembleDebug green (26.0 MB APK). Branch `agent/p13-device-round-fixes`. D93/D94 drafts re-issued for owner paste.
...
- [x] **P12 DELIVERY update (2026-09-24):** PR #71 (Part A) MERGED → main @ `cad3199` (CI green). PR #72 (P12) was then merged — but into its stacked base branch `agent/part-a-terminology-reland` (`801a4bc`), NOT main: **P12 was never on main**; no CI ran for that merge (ci.yml push-trigger is main-only). Logged as ERR-019 (stacked-PR base not retargeted before owner merged). P12 re-landed as **PR #73 (base main ← `agent/p12-multi-line-payments`)** — identical content to the CI-green #72 head plus docs (ERR-019 + this entry). Owner: merge #73 and P12 lands on main.
- [ ] **Exit-gate:** B-001/B-002/B-003/B-004 verified on Sakira device (2026-09-18); B-005/B-006 verified by table-level tests pending device re-test; PR #68 device round (2026-09-19): B-011 ক্যাশবুক ✅, B-012 তোলা ✅, U-002 পূর্বের বাকি ✅, FAB colors ✅; B-009 device re-test + D88 header ✅; remaining: screenshot tests green in CI + B-005/B-006 device re-test + normal-sale regression re-run + B-013/B-014 device re-test (expense chips after fresh install/relaunch; add-book required marks + Bangla-digit prices) + C-round device checks (supplier payment + cash-close inputs + subscription amount with Bangla digits) + **Part A device sweep after PR #71 merge+reinstall (উপমুট gone from POS summary, দেনা-পাওনা button, স্থূল মুনাফা in reports, নগদ (Nagad) on receipts)** + D91/D92 pastes + **P12 device checks (Migration6To7 against live pilot data; 2-way split ৳600 নগদ + ৳400 মোবাইল → zero বাকি; 3-way with খাতা remainder; receipt জমা rows; cash-close bucket rows)**. **NOTE: P12 implementation started 2026-09-24 BEFORE this gate closed — explicit owner override (see the P12 IMPLEMENTED entry); the gate items above remain open and mandatory**
...
| `agent/p11-c1-parse-cleanup-catalog-failfast` | #70 | C-1..C-5 parse-cleanup (supplier/cash-close/subscription/mela) + D86 catalog write fail-fast; Part A audit + Part B payment-model proposal delivered; ERR-016 | **Merged 2026-09-24** (merged at `f1bce2d` — did NOT include the never-pushed Part A commits) |
| `agent/part-a-terminology-reland` | #71 MERGED 2026-09-23 → main @ `cad3199` (CI green) | Part A terminology re-land (A1–A5) + discount-branch RED→GREEN; ERR-018 | Merged; branch later carried the misplaced #72 merge (801a4bc) — see ERR-019 |
| `agent/p12-multi-line-payments` | #72 merged into WRONG BASE 2026-09-24 → re-landed as **PR #73 (base main)** | P12/D92 multi-line payment model: schema v7 + D22 extension + per-line cashbook + MOBILE bucket + POS multi-line UI + all consumers + backup/restore | #72 was merged into `agent/part-a-terminology-reland` (801a4bc) — **P12 was NOT on main**; ERR-019; re-opened as PR #73 with identical content |
| `agent/p13-device-round-fixes` | PR #7X (open) | P13 device round: D93 overpayment→খাতা জমা (D22 in-transaction), D94 revenue/COGS + labels, restore-time khata repair, POS overpay preview | CI pending (this PR) |
