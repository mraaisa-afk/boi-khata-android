...
| `agent/part-a-terminology-reland` | push-pending | Part A terminology re-land (A1–A5) + discount-branch RED→GREEN; ERR-018 | Code+docs committed locally — **push blocked: no token this session** |
| `agent/p12-multi-line-payments` | push-pending (stacked on the Part A branch) | P12/D92 multi-line payment model: schema v7 + D22 extension + per-line cashbook + MOBILE bucket + POS multi-line UI + all consumers + backup/restore | Code+docs committed locally — **push blocked: no token this session** |
