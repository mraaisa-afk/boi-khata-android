...
- [x] ড্যাশবোর্ড-ফিক্স: আজকের বিক্রি কার্ড + actionable empty state + digit law + WhatsApp share
  - নোট: PR #40 (agent/p6-dashboard-fix) — HomeScreen: TodaySalesCard composable যোগ (todaySalesTotal+todayBillCount ViewModel-এ ছিল কিন্তু UI-তে render হচ্ছিল না); EmptyDueState actionable 2-line Bengali message; banglaDigit() helper — age_days/due_customers/supplier_count/today_bills সব Bengali numeral (%1$d→%1$s+banglaDigit); "সাপ্লায়ার পাওড়া"→"পাওনা" typo fix; "top ৫"→"শীর্ষ বাকিদার" mixed-language fix। AndroidManifest: <queries> ACTION_SEND text/plain tag যোগ — WhatsApp/share chooser Android 11+-এ এখন কাজ করবে। CI ✅ merged ✅.
...
- [x] রিলিজ: R8+সাইনড-APK+ভার্সন-সানসেট-ম্যানিফেস্ট + এজেন্ট-APK-চ্যানেল
  - নোট: P8 added R8 keep rules, catalog-managed version 0.8.0/code 8, changelog, and a minified release APK. Direct APK update check remains local-policy-ready; vendor download/contact endpoint is intentionally not fabricated.
- [x] রেফারেল+কো-সেল-কিট; ফাউন্ডার্স-ক্লাব-অনবোর্ডিং
  - নোট: Added deterministic tenant referral code, vendor-validation messaging, Founders Club ৳150/month / first-50 policy, settings cards, and Bengali pilot onboarding checklist.
