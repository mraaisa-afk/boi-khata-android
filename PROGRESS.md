...
- [x] D81 formula fix prepared in PR #51: `paidAmount` + khata PAYMENT collection included in HomeScreen net-profit formula
- [ ] Per-screen device audit
- [ ] **Exit-gate:** screenshot test at max font-scale
  - নোট: PR #51 open on `agent/p10-d81-formula-fix`; PROGRESS.md conflict resolved after PR #52 workflow merge. CI must rerun and pass before owner merge.
...
| `agent/p10-d81-formula-fix` | #51 | P10 D81 formula fix | Conflict resolved; CI rerun required |
| `agent/workflow-exact-stacked-pr` | #52 | Workflow doc alignment | Merged |
