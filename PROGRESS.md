...
- [ ] **Part D systemic audit (2026-09-19) — INVENTORY ONLY, needs owner go-ahead before any fix (per owner instruction):** remaining RAW (un-normalized) ASCII parses on user input, found by repo-wide grep after this round's fixes — (1) `SupplierScreen.kt:408–409` supplier payment: gate + onClick BOTH raw → identical gray-সেভ symptom as B-012 on Bangla digits (money write, HIGHEST priority of the four); (2) `MelaScreen.kt:293–294` mela stock move: gate + onClick raw → same gray-সেভ class (inventory write); (3) `CashCloseScreen.kt:70/81` MFS rate + counted cash: no gate, raw parse → silent ৳0 corruption of close records; (4) `SubscriptionScreen.kt:154` manual bKash amount: no gate, raw parse → silent ৳0 payment record. Seed-dependency sweep: expense_categories was the ONLY silently-empty seed data (fixed in B-013); books/khata/suppliers/mela are user-created. D86 write-path sweep: 6 VMs still carry pre-load `currentTenantId = "t_1"` fallbacks (Sale:56, Expense:38, Supplier:41, Mela:29, CashClose:36, Reports:59 — writes are preceded by mandatory load*(tenantId) in practice) and `CatalogViewModel.addBook` lacks a blank-tenant fail-fast (B-004 discipline). RECOMMENDATION: one dedicated cleanup round (P11 or P12 opener) — ~6 small normalizer patches + 6 VM sentinel swaps, all test-backed; no architectural risk identified
...
- [ ] **Exit-gate:** B-001/B-002/B-003/B-004 verified on Sakira device (2026-09-18); B-005/B-006 verified by table-level tests (40→37 stock, ledger/cashbook/khata rows) pending device re-test; PR #68 device round (2026-09-19): B-011 ক্যাশবুক ✅, B-012 তোলা ✅, U-002 পূর্বের বাকি ✅, FAB colors ✅, B-010 expense — data-layer failure found (B-013) and re-fixed this round; B-009 device re-test + D88 header ✅ (device screenshot); remaining: screenshot tests green in CI + B-005/B-006 device re-test + normal-sale regression re-run (blocked this round by B-013's empty-category data state, not by sale code) + B-013/B-014 device re-test (expense chips appear after fresh install/relaunch; add-book shows required marks + accepts Bangla-digit prices) + Part D cleanup round go/no-go
