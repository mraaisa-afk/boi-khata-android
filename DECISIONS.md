...
**Date:** 2026-09-04
**Phase:** 7b
**Context:** P7b turns the policy layer into device behavior: trial redemption must survive process restarts, monthly CSV generation must be safe under Android background-launch restrictions, and the pilot artifact must be reproducible without introducing Firebase services.
**Decision:** Add a Room `trial_redemptions` table with tenantId, deviceFingerprint, phoneHash, and redeemedAt, migrated additively from schema v4 to v5 and included in tenant rebind scope. Trial usage is read from existing Room bill/book counts; cap decisions remain policy-driven and writes continue through the existing `LicenseWriteGuard`. The monthly copy worker generates an app-scoped CSV and records its path in local preferences; the foreground settings surface performs the ACTION_SEND share-sheet handoff. Voice setup and Lite mode are persisted in per-user app preferences, not business tables. Release verification uses the existing debug signing configuration when a supplied keystore is present; absent a vendor key, the unsigned release output is reported rather than mislabeled as a distributable pilot APK.
**Alternatives considered:** SharedPreferences for redemption (rejected: the requirement explicitly calls for Room persistence); launching a share sheet from the worker (rejected: Android background activity-launch restrictions and D60); a new cloud trial or migration collection (rejected: no new Firebase and claims remain vendor-side); silently using a generated release key (rejected: it would not be the vendor's trusted signing identity).
**Supersedes:** —
...
**Date:** 2026-09-04
**Phase:** 8
**Context:** P8 adds production-readiness controls without changing Firebase’s locked scope. Prior P6/P7 device-only gates are intentionally carried into the 20-shop pilot rather than falsely marked green in the sandbox.
