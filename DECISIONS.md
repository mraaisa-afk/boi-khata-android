...
## D86 — Write-path tenant threading: per-destination ViewModels receive tenantId explicitly; seed-tenant "t_1" write-fallbacks are forbidden (fixes B-004)
**Date:** 2026-09-18
**Phase:** P11
**Context:** Sakira device report after PR #63: newly added khata customers appeared only after an app restart. The B-002 reactive read chain (D83) was verified correct — `getActiveByTenantFlow` collector alive across the whole add-customer navigation. The defect was on the write side: `khata_add_customer` scopes its own `KhataViewModel` instance (hiltViewModel per back-stack entry) which never runs `loadCustomers`, so `addCustomer` wrote with the hardcoded fallback `currentTenantId = "t_1"` while the list Flow filters on the claims tenant. The D41 rebind — which runs on EVERY launch because `MainViewModel` passes the hardcoded literal `"t_1"` into `ClaimsSession.needsRebind` — silently migrated the stray row at next launch, masking the corruption as a "restart fixes it" quirk.
**Decision:**
1. Every composable destination that performs writes must receive `tenantId` as an explicit parameter from the NavHost scope and thread it into the ViewModel call — never rely on VM-side mutable state that a fresh per-destination instance has not initialized. Applied: `khata_add_customer` (KhataAddCustomerScreen(tenantId) → addCustomer(tenantId, ...)).
2. ViewModel write-methods must NOT fall back to the seed tenant `"t_1"`. The KhataViewModel field default is now `""` (blank sentinel); a write with a blank tenant fails fast with a visible Error state instead of inserting invisible rows. Any `"t_1"` literal outside `DatabaseSeeder.SEED_TENANT_ID`, the D41 rebind call site, and tests requires a fresh D-ruling.
3. Related defect flagged (P11 follow-up, needs owner ruling — NOT changed in this fix): `MainViewModel.resolveAuthState` hardcodes `oldTenantId = "t_1"` so `needsRebind` is trivially true and `rebind` runs on every launch. This per-launch rebind is the safety net that rescued B-004's stray rows; making it one-time-persisted (read the actual local tenant from `cloud_sync_state` instead of the literal) would surface future mis-tenant writes instead of masking them, but changes migration semantics and must not ride along a bugfix PR.
4. Audit record for the sibling `"t_1"` defaults (SaleViewModel, ExpenseViewModel, SupplierViewModel, MelaViewModel, CashCloseViewModel, ReportsViewModel): each is assigned by the screens' mandatory `load*(tenantId)` before any write path is reachable — no change required now; the D86 rule 1 governs any future refactor of these screens.
**Alternatives considered:** calling `loadCustomers(tenantId)` from the add screen on its own VM instance (rejected: starts an unneeded collection and papers over the missing-explicit-argument smell — the write would still depend on hidden init order); scoping one shared KhataViewModel to the activity (rejected: broad blast radius for a state machine that is intentionally per-destination; revisit only with an owner ruling); persisting "last used tenant" in DataStore (rejected: yet another stale-state source — the NavHost already owns the truth).
**Supersedes:** —

---

## D87 — Picker sheets are search-as-filter: the existing dataset loads on open; blank-query = full-list contract; empty/loading states are mandatory (fixes U-001)
**Date:** 2026-09-18
**Phase:** P11
**Context:** Owner device feedback 2026-09-18 after PR #64: in the POS screen, «ক্রেতা > নির্বাচন» opened a buyer sheet with only a search box and NO existing-customer list, and the empty cart's «বই যোগ করুন» opened a book sheet with the same blank-until-typed behavior. The owner explicitly asked for the existing customer (and book) lists to be visible alongside the search box. Root cause was UI wiring, not data: both `searchBooks` and `searchCustomers` repos already treat a blank query as a full-list request (`searchBooks("")` → `getBooks`, `searchCustomers("")` → `getCustomers` per KhataRepositoryImpl/BookRepositoryImpl), but the sheets only fired the query from the search box's `onValueChange`, so results were empty exactly when the user needed orientation the most.
**Decision:**
1. Any picker/search sheet over a bounded local dataset (Room, single tenant) opens with the FULL list rendered and treats the search box as a live FILTER — not as a discovery precondition. Implemented for POS: `BookSearchSheet` and `CustomerPickerSheet` fire `onSearch("")` via `LaunchedEffect(Unit)` on open.
2. Blank-query = full-list is now a repo-layer contract: every `search*(tenantId, "")` MUST return the full active dataset for the tenant. Any future search implementation that special-cases blank to empty requires a new D-ruling.
3. Search state is a sealed UI state (Idle/Loading/Success/Error) with a mandatory visible branch for each: spinner on load, error text on failure, and an explicit empty-state string (`no_books_found` / `no_customers_found`) — a silently blank result area is forbidden. Registered as the U-register (UX gaps): U-001 is the first entry; UX-gap fixes follow the same traceable-ID + branch + PR flow as B-register bugs.
4. Keystroke searches cancel the previous query job before launching (`bookSearchJob`/`customerSearchJob` in SaleViewModel, mirroring KhataViewModel's B-002 pattern) — an out-of-order slow response must never overwrite a newer result.
5. Follow-on from D86: `SaleViewModel.checkout` now takes `tenantId` explicitly from the PosScreen destination with a blank-tenant fail-fast, completing D86 rule 1 for the sale module's primary write path.
**Alternatives considered:** debounced search with empty initial state (rejected: still blank on open — the exact complaint; debounce adds latency with no local-Room benefit at pilot data volumes); pre-warming search results in `init {}` on VM creation (rejected: queries fire before the sheet is wanted and fight D86 — tenant arrives with the destination, not at VM construction); paging the list (rejected: YAGNI at pilot scale, revisit if catalogs exceed hundreds of rows).
**Supersedes:** —
